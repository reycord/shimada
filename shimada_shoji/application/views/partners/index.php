        ...
        <div class="x_panel">
            <div class="x_title">
                ...
                <ul class="nav navbar-right panel_toolbox">
                    <li><button class="btn btn-primary" onclick="window.location.href='<?php echo base_url(); ?>partners/add/1'"><i class="fa fa-plus-square"></i> <?php echo $this->lang->line('new_partner'); ?></button></li>
            ...
            <div class="x_content">
                <div class="table-responsive">
                    <table id="partners_list" class="table table-striped table-bordered cssTable display nowrap" width="100%" cellspacing="0">
                        <thead>
                            <tr>
...
<div class ="hidden-form">
    <form id="export_excel_form" action = "<?php echo base_url('partners/excel')?>" method="GET">
    </form>
</div>
...
<script>
    ...
    window.onload = function() {
        var partnerDt = $('#partners_list').DataTable({
            "data": [],
            "dom": "lBfrtip",
            "buttons": ['colvis'],
            "paging": true,
            ...
            <?php endif?>
            "ajax": { "url": "<?php echo base_url('partners/search'); ?>" },
            "columns": [
                ...
                { "data": "company_name", className: "text-left",
                    "render": function( data, type, row, meta ) {
                        return '<a class="edit" href="<?php echo base_url(); ?>partners/edit/'+ row.company_id +'" style="color:#428bca;">'+row.company_name+'</a>'
                    }
                },
        ...
        $('#partners_list_filter label input[type=search]').val('Default Product')

        $("#partners_list").on('click', '#btn_item_detail', function(){
            ...
            $.ajax({
                url: "<?php echo base_url('partners/get_item_list')?>",
                data: requestData,
                type: 'post',
                dataType: 'json',
        ...
        $('#btn_export_excel').on('click', function() {
            $('#frm_search_product').attr("action",'<?php echo base_url("partners/excel") ?>');
        ...
        $('#add_record').click(function(e) {
            ...
            if(item_code != '') {
                ...
                if (findItem) {
                    bootbox.alert('<h4>Item is exist</h4>');
                    return;
                }
                ...
                $.ajax({
                    url: "<?php echo base_url('partners/update_item_list')?>",
                    type: 'post',
                    data: requestData,
                    dataType: 'json',
                }).done(function (response) {
                    ...
                    if(response.success) {
                        $('#partners_list').DataTable().ajax.reload(null, false);
                    } else {
                        $('#partners_list').DataTable().ajax.reload(null, false);
