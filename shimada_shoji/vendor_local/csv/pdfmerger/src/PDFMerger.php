...
class PDFMerger
{
	 ...
	 * @param $filepath
	 * @param $pages
	 * @param $buffer -- buffer is true when $filepath is pdf data
	 * @return void
	 */
	public function addPDF($filepath, $pages = 'all', $buffer = false)
	{

		if($buffer || file_exists($filepath))
		{
			...
			$this->_files[] = array($filepath, $pages, $buffer);
		}
		else
		{
    ...
    $fpdi->SetPrintFooter(false);
		...
		foreach($this->_files as $file)
		{
			...
			$buffer = $file[2];
			if($buffer){
				$count = $fpdi->setSourceData($filename);
			}else{
			...
			else
			{
				...
				{
					if(!$template = $fpdi->importPage($page)): throw new exception("Could not load page '$page' in PDF '".($buffer?'pdfData':$filename)."'. Check that the page exists."); endif;
					$size = $fpdi->getTemplateSize($template);
					$orientation = ($size['h'] > $size['w']) ? 'P' : 'L';
